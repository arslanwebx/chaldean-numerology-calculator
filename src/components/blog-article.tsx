...
export function BlogArticle({ title, description, path, published, modified = published, image, imageAlt, imageWidth = 1200, imageHeight = 630, sections, faqs, children }: { title: string; description: string; path: string; published: string; modified?: string; image: string; imageAlt: string; imageWidth?: number; imageHeight?: number; sections: { id: string; label: string }[]; faqs: readonly Faq[]; children: ReactNode }) {
  ...
  return <>
    ...
    <ContentPage title={title} intro={description} path={path} sections={sections} variant="article">
      ...
      <figure className="article-featured"><Image src={image} alt={imageAlt} width={imageWidth} height={imageHeight} priority sizes="(max-width: 780px) calc(100vw - 1rem), 760px" /></figure>
      {children}
