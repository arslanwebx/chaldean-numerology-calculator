...
export function ContentPage({ title, intro, path, children, sections = [], variant = "default" }: { eyebrow?: string; title: string; intro: string; path: string; children: ReactNode; sections?: { id: string; label: string }[]; variant?: "default" | "archive" | "article" }) {
  const tocSections = sections.length > 0 ? sections : articleTocs[path] ?? [];
  const widthClass = variant === "archive" ? "content-wide" : variant === "article" ? "content-reading" : "narrow";
  return <div className={`content-page content-page-${variant}`}><JsonLd data={[{ "@context": "https://schema.org", "@type": "WebPage", name: title, description: intro }, breadcrumbJson([{ name: "Home", path: "/" }, { name: title, path }])]} /><header className="page-hero"><div className={widthClass}><p className="breadcrumbs"><Link href="/">Home</Link> / {title}</p><h1>{title}</h1><p className="lede">{intro}</p>{tocSections.length > 0 && <details className="toc"><summary>Contents</summary><nav aria-label="Table of contents"><ol>{tocSections.map((section) => <li key={section.id}><a href={`#${section.id}`}>{section.label}</a></li>)}</ol></nav></details>}</div></header><section className="section"><div className={widthClass}><article id="content" className="prose">{children}</article></div></section></div>;
}
