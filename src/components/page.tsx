...
export function ContentPage({ eyebrow, title, intro, path, children, sections = [] }: { eyebrow: string; title: string; intro: string; path: string; children: ReactNode; sections?: { id: string; label: string }[] }) {
  const showToc = sections.length > 0 && path !== "/blog/";
  return <><JsonLd data={[{ "@context": "https://schema.org", "@type": "WebPage", name: title, description: intro }, breadcrumbJson([{ name: "Home", path: "/" }, { name: title, path }])]} /><header className="page-hero"><div className="narrow"><p className="breadcrumbs"><Link href="/">Home</Link> / {title}</p><p className="eyebrow">{eyebrow}</p><h1>{title}</h1><p className="lede">{intro}</p>{showToc && <nav className="toc" aria-label="Table of contents"><strong>On this page</strong><ol>{sections.map((section) => <li key={section.id}><a href={`#${section.id}`}>{section.label}</a></li>)}</ol></nav>}</div></header><section className="section"><div className="narrow"><article id="content" className="prose">{children}</article></div></section></>;
}
