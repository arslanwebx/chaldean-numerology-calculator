...
The launch archive contains zero posts. Metadata lives in `src/content/posts.ts`; only entries with `draft: false` may be listed or added to the sitemap.
...
5. When the first non-draft post exists, list it on `/blog/`, remove the archive's noindex setting, and derive its sitemap inclusion from `publishedPosts`.
